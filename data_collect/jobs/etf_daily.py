"""ETF 日线采集（不复权，写 etf_daily_kline）。镜像 a_share_daily。"""
...
def _missing_codes(trade_date: str, codes: list) -> list:
    with get_connection() as conn:
        existing = pd.read_sql(
            "SELECT DISTINCT code FROM etf_daily_kline WHERE trade_date = %s",
            conn, params=[trade_date])
    have = set(existing["code"])
    return [c for c in codes if to_bare_code(c) not in have]
...
def run(run_date: str, **kwargs) -> str:
    ...
    if not is_market_day(trade_date):
        return f"{trade_date} 非交易日，ETF日线跳过。"
    codes = _get_codes(kwargs.get("limit_stocks"))
    with tqdm(total=len(codes), desc=f"ETF日线({trade_date})", unit="只", file=sys.stdout) as pbar:
        tried, inserted = _collect_one_day(trade_date, codes, pbar)
    return f"{trade_date} ETF日线完成，写入 {tried}/{inserted} 条。"
...
def run_backfill(start_date: str, end_date: str, limit_stocks=None) -> str:
    ...
    for i, d in enumerate(days, 1):
        with tqdm(total=len(codes), desc=f"{i}/{len(days)} {d}", unit="只", file=sys.stdout) as pbar:
            t, ins = _collect_one_day(d, codes, pbar)
            tot_t += t; tot_i += ins
    msg = f"ETF日线补历史完成 ({start_date}~{end_date})，{len(days)} 交易日，写入 {tot_t}/{tot_i} 条。"
    send_dingtalk(msg)
    return msg
...
def run_verify(start_date: str, end_date: str, **kwargs) -> str:
    ...
    for i, d in enumerate(days, 1):
        ...
        if not missing:
            continue
        fixed += 1
        with tqdm(total=len(missing), desc=f"补缺 {i}/{len(days)} {d} 缺{len(missing)}", unit="只", file=sys.stdout) as pbar:
            t, ins = _collect_one_day(d, missing, pbar)
            tot_t += t; tot_i += ins
    if fixed == 0:
        return f"ETF日线检查完成，{len(days)} 交易日完整（{len(codes)}只）。"
    return f"ETF日线补缺完成，{fixed}/{len(days)} 天有缺，补 {tot_t}/{tot_i} 条。"
